...
export function run(h: SimHarness): void {
  ...
  h.tick();
  expectTrue(hero.dodgeLevel === 1 && hero.skillPoints === 0, 'W rank 1 learned at level 1');
  ...
  expectTrue(hero.abilityLevel === 3, `Q capped at rank 3 at level 5 (got ${hero.abilityLevel})`);
  ...
  h.issue('p1', { type: 'levelAbility', ability: 'blast' });
  h.tick();
  expectTrue(hero.blastLevel === 1, 'R rank 1 available at level 6');
  h.issue('p1', { type: 'levelAbility', ability: 'blast' });
  h.tick();
  expectTrue(hero.blastLevel === 1 && hero.skillPoints === 1, 'R rank 2 blocked until 11');

  // Threshold, not a window: E rank 1 (skipped so far) still learnable later.
  hero.level = 7;
  h.issue('p1', { type: 'levelAbility', ability: 'reveal' });
  h.tick();
  expectTrue(hero.revealLevel === 1, 'skipped ability still learnable later');
  ...
  h.issue('p1', { type: 'levelAbility', ability: 'blast' });
  h.tick();
  expectTrue(hero.blastLevel === 2, 'R rank 2 at level 11');
  hero.level = 16;
  h.issue('p1', { type: 'levelAbility', ability: 'blast' });
  h.tick();
  expectTrue(hero.blastLevel === 3, 'R rank 3 at level 16');
  h.issue('p1', { type: 'levelAbility', ability: 'blast' });
  h.tick();
  expectTrue(hero.blastLevel === 3 && hero.skillPoints === 1, 'R capped at rank 3');
  ...
  expectTrue(hero.abilityLevel === 5, 'Q reaches rank 5 at high level');
