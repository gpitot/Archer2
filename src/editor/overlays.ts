...
export class Overlays {
  ...
  setGhost(kind: 'tree' | 'rock' | 'deco' | 'camp' | 'spawn' | 'rune' | 'shop' | 'castle' | null, radius: number): void {
    ...
    const mat = new THREE.MeshBasicMaterial({
      color: kind === 'camp' ? 0xcc4444 : kind === 'spawn' ? 0x4488cc : kind === 'rune' ? 0xffcc33 : kind === 'shop' ? 0xffcc44 : kind === 'castle' ? 0xaa88ff : 0x66cc66,
      transparent: true,
      opacity: 0.5,
      depthWrite: false,
    });
